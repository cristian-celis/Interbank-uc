...
| 05 | `05_DML_app_cliente_extras_core_mobile.sql` | Extras para app de clientes: tarjetas y notificaciones demo. |
| 06 | `06_DDL_workflow_creditos.sql` | Workflow de crédito: decisiones de comité. |
| 07 | `07_DML_cuentas_movimientos_demo.sql` | Cuentas y movimientos demo. |
| 08 | `08_DML_30_casos_credito_flujo_movil.sql` | **30 casos de crédito empresarial** del flujo móvil: clientes, solicitudes, buró, comité, desembolso, cronograma, documentos y notificaciones. |
| 99 | `99_run_all.sql` | Runner que ejecuta 01→08 en orden con `psql`. |
...
> desde pgAdmin (Query Tool → abrir archivo → Run), respetando el orden 01→08.
...
- **30 casos de originación móvil** con expedientes `FM-001`…`FM-030`:
  **24 aprobados**, **3 condicionados**, **3 rechazados**, **27 créditos** `CRED-FM-*`,
  **510 cuotas**, **30 consultas de buró**, **30 decisiones de comité** y
  **150 documentos placeholder**.
